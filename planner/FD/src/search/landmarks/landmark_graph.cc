...
void LandmarkGraph::dump_nodea(const LandmarkNode *node_p) const {
    ofstream myfile;
    myfile.open ("landmark.txt", std::ios_base::app);
    cout << "LM " << node_p->get_id() << " ";
    myfile << node_p->get_id() << " ";
	...
	myfile << g_fact_names[var_no][value]<<" ";
